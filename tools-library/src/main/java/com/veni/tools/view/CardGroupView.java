...
public class CardGroupView extends RelativeLayout{
    ...
    public void addView(View card) {
        if (isLoadMore) {
            this.mCardList.add(mCardList==null?0:mCardList.size(), card);
        } else {
    ...
    private void setLayoutParams(View card, int index) {
        ...
        params.topMargin = (int) (DeviceTools.getDisplayMetrics(mContext).heightPixels * margin) + getResources().getDimensionPixelSize(
                R.dimen.card_item_margin) * index;
        params.bottomMargin = (int) (DeviceTools.getDisplayMetrics(mContext).heightPixels * margin) - getResources().getDimensionPixelSize(
                R.dimen.card_item_margin) * index;
        params.leftMargin = (int) (DeviceTools.getDisplayMetrics(mContext).widthPixels * margin);
        params.rightMargin = (int) (DeviceTools.getDisplayMetrics(mContext).widthPixels * margin);
            ...
            if (mOnTouch && v.equals(mCardList.get(0))) {
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        mRightOut = mCardList.get(0).getLeft() > DeviceTools.getDisplayMetrics(mContext).widthPixels / 2;
                        mLeftOut = mCardList.get(0).getRight() < DeviceTools.getDisplayMetrics(mContext).widthPixels / 2;
                        mLastY = rawY;
                        mLastX = rawX;
                        break;
    ...
    private void rightOut() {
        CardIndex oldCardIndex = new CardIndex(DeviceTools.getDisplayMetrics(mContext).widthPixels, mCardTop, DeviceTools.getDisplayMetrics(mContext).widthPixels + (mCardRight - mCardLeft), mCardBottom);
        CardIndex newCardIndex = new CardIndex(mCardList.get(0).getLeft(), mCardList.get(0).getTop(), mCardList.get(0).getRight(), mCardList.get(0).getBottom());
        animator(newCardIndex, oldCardIndex);
    }
    ...
    public void removeTopCard() {
        if (!DataTools.isEmpty(this.mCardList)) {
